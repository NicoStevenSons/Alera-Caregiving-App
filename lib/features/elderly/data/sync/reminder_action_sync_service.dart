...
class ReminderActionSyncService {
  ...
  Future<void> syncPendingActions({
    ...
    final actions =
        await localService.getPendingReminderActions();

    debugPrint(
      'Found ${actions.length} pending reminder actions',
    );
    ...
    for (final action in actions) {
      ...
      try {
        await _syncOneAction(
          action,
          performedByUserId,
        );

        await localService.deleteReminderAction(
          localId,
        );

        debugPrint(
          'Synced reminder action $localId',
        );
      } catch (error) {
        await localService.markReminderActionFailed(
          localId,
          error.toString(),
        );
  ...
  Future<void> _syncOneAction(
    ...
    final String occurrenceId =
        action['occurrence_id'] as String;

    final String actionType =
        action['action_type'] as String;

    final String? previousStatus =
        action['previous_status'] as String?;

    final String? newStatus =
        action['new_status'] as String?;

    final String? newDueAt =
        action['new_due_at'] as String?;

    final String? actionNote =
        action['action_note'] as String?;
