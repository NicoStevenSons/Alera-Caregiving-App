...
class ReminderLocalService {
    ...
    for (final reminder in reminders) {
      batch.insert(
        'reminder_cache',
        {
          ...
          'snooze_allowed':
              reminder.snoozeAllowed ? 1 : 0,
          'default_snooze_minutes':
              reminder.defaultSnoozeMinutes,
          'synced_at':
  ...
  Future<List<ElderlyReminder>> getReminders(
    String patientId,
  ) async {
    final db = await AleraDatabase.instance.database;
    ...
    return rows.map((row) {
      return ElderlyReminder(
        ...
        snoozeAllowed:
            (row['snooze_allowed'] as int) == 1,
        defaultSnoozeMinutes:
            row['default_snooze_minutes'] as int,
      );
    }).toList();
  }
  ...
  final bool isLate =
      DateTime.now().isAfter(reminder.dueAt);
 ...
 Future<DateTime> snoozeReminder(
  ElderlyReminder reminder,
) async {
  final db = await AleraDatabase.instance.database;
  ...
  final DateTime newDueAt = DateTime.now().add(
    Duration(
      minutes: reminder.defaultSnoozeMinutes,
    ),
  );
 ...
 Future<ElderlyReminder?> getReminder(
  String occurrenceId,
) async {
  final db = await AleraDatabase.instance.database;
 ...
 Future<void> deleteReminderAction(
  ...
  Future<void> markReminderActionFailed(
  int id,
  String error,
) async {
  final db = await AleraDatabase.instance.database;
